use bevy::{
    core::Time,
    ecs::{Commands, IntoSystem, Query, Res, ResMut},
    math::Vec3,
    pbr::PbrBundle,
    prelude::{
        AppBuilder, AssetServer, Assets, BuildChildren, Color, Handle, Mesh, StandardMaterial,
        Transform,
    },
};
...
impl bevy::prelude::Plugin for Plugin {
    fn build(&self, app: &mut AppBuilder) {
        app.add_startup_system(create.system())
...
impl Piece {
    ...
    pub fn is_move_valid(&self, new_position: (u8, u8), pieces: Vec<Piece>) -> bool {
        ...
        if color_of_square_helper(new_position, &pieces) == Some(self.color) {
            return false;
        }
        ...
        match self.piece_type {
            ...
            PieceType::Queen => {
                is_path_empty_helper((self.x, self.y), new_position, &pieces)
            ...
            PieceType::Pawn => {
                if self.color == PieceColor::White {
                    ...
                    if new_position.0 as i8 - self.x as i8 == 1 && (self.y == new_position.1) {
                        if color_of_square_helper(new_position, &pieces).is_none() {
                            return true;
                        }
                    }
                    ...
                    if new_position.0 as i8 - self.x as i8 == 1
                        && (self.y as i8 - new_position.1 as i8).abs() == 1
                    {
                        if color_of_square_helper(new_position, &pieces) == Some(PieceColor::Black)
                        {
                            return true;
                        }
                    }
                    ...
                    if new_position.0 as i8 - self.x as i8 == -1
                        && (self.y as i8 - new_position.1 as i8).abs() == 1
                    {
                        if color_of_square_helper(new_position, &pieces) == Some(PieceColor::White)
                        {
                            return true;
                        }
                    }
...
fn create(
    ...
    // Add some materials
    let white_material = materials.add(Color::rgb(1., 0.7, 0.7).into());
...
fn color_of_square_helper(pos: (u8, u8), pieces: &Vec<Piece>) -> Option<PieceColor> {
    for piece in pieces {
...
fn is_path_empty_helper(begin: (u8, u8), end: (u8, u8), pieces: &Vec<Piece>) -> bool {
    // Same column
    if begin.0 == end.0 {
        for piece in pieces {
            if piece.x == begin.0
    ...
    if x_diff == y_diff {
        for i in 1..x_diff {
            ...
            if color_of_square_helper(pos, pieces).is_some() {
                return false;
            }
        }
    }
